import cfnlint.core
from cfn_mp_ql_rules import IAMPartition
...
def return_rule_line_matches(filename):
    ...
    (_, rules, __) = cfnlint.core.get_template_rules(
        fn, cfnlint.config.ConfigMixIn([])
    )
    rule_matches = cfnlint.core.run_checks(
        fn, cfn.template, rules, ["us-east-1"]
    )
    matches_dict = {}
